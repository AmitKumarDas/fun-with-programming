#### References
```yamlParser
- https://github.com/gavinbunney/terraform-provider-kubectl/blob/master/yaml/splitter.go
```
...
func SplitMultiDocumentYAML(multidoc string) (documents []string, err error) {
	...
	var maxCapacity = bufio.MaxScanTokenSize // TIL
	for _, element := range contentSplit {
		if len(element) >= maxCapacity { //set max as the size of the largest item
			maxCapacity = len(element) + 100
		}
	}

	scanner := bufio.NewScanner(bytes.NewReader(content)) // entire content
	...
	scanner.Split(splitYAMLDocument) // provide the callback
...
func splitYAMLDocument(data []byte, atEOF bool) (advance int, token []byte, err error) {
	...
	if i := bytes.Index(data, []byte(yamlSeparator)); i >= 0 {
		...
		if len(after) == 0 {
			...
			if atEOF {
				return len(data), data[:len(data)-sep], nil // remove separator from data
			}
			return 0, nil, nil
		}
