...
==== Source Code - Deserialize - Attemp 1
[source, go]
----
func Deserialize(ser string) *BT {
  if ser == "" {
  ...
  var root, tmp *BT
  splits = strings.Split(ser, ",")
  for i:=0; i<len(splits); i+=3 {
    if i == 0 {
      root = &BT{Val: int(splits[i])}
      tmp = root
    } else {
      tmp = &BT{Val: int(splits[i])}
    }
    
    if i+1 < len(splits) && splits[i+1] != "null" {
      tmp.Left = &BT{Val: int(splits[i+1])}
    }
    
    if i+2 < len(splits) && splits[i+2] != "null" {
      tmp.Right = &BT{Val: int(splits[i+2])}
    }
